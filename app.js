...
const defaultData = {
 profile: {
  ...
  about: "I combine agriculture, digital marketing, and graphic design to build practical projects and useful digital experiences.",
...
function render() {
 ...
 $("#timeline").innerHTML = data.timeline.map(x => `
  <article class="timeline-item">
   <p class="eyebrow">${esc(x.type)}</p>
   <h3>${esc(x.title)}</h3>
   <div class="muted">${esc(x.period)}</div>
   <p>${esc(x.description)}</p>
  </article>
 `).join("");
 
 $("#certGrid").innerHTML = data.certificates.length ? data.certificates.map(x => `
  <article class="card">
   <img class="card-img" src="${safeUrl(x.image)}" alt="">
   <div class="card-body">
    <h3>${esc(x.title)}</h3>
    <p class="muted">${esc(x.issuer || "")}</p>
    <p>${esc(x.description || "")}</p>
   </div>
  </article>
 `).join("") : `<p class="muted">Certificates will appear here.</p>`;
...
function renderProjects(cat) {
 const arr = cat === "All" ? data.projects : data.projects.filter(x => x.category === cat);
 $("#projectsGrid").innerHTML = arr.map(x => `
  <article class="card">
   <img class="card-img" src="${safeUrl(x.image)}" alt="">
   <div class="card-body">
    <p class="eyebrow">${esc(x.category || "Project")}</p>
    <h3>${esc(x.title)}</h3>
    <p>${esc(x.description)}</p>
    ${x.url && x.url !== "#" ? `<a class="btn" href="${safeUrl(x.url)}" target="_blank">Open project</a>` : ""}
   </div>
  </article>
 `).join("");
}
...
$("#projectFilters").addEventListener("click", e => {  if(e.target.matches(".filter")) {   $$(".filter").forEach(x => x.classList.remove("active"));
...
function buildEditor() {
 ...
 <label>Tagline<input id="eTagline"></label>
 <label>About<textarea id="eAbout" rows="4"></textarea></label>
 <label>Profile photo<input id="profileFile" type="file" accept="image/*"></label>
 <img id="profilePreview" class="upload-preview"></div>
 ...
 <div class="editor-section"><h3>Contact</h3><div class="row">
 <input id="eEmail" placeholder="Email">
 <input id="ePhone" placeholder="Phone">
 <input id="eFacebook" placeholder="Facebook URL">
 <input id="eLinkedin" placeholder="LinkedIn URL">
 <input id="eGithub" placeholder="GitHub URL">
 </div><textarea id="eContactText"></textarea></div>
...
function fillEditor() {
 $("#eName").value = data.profile.name; $("#eTagline").value = data.profile.tagline;
 $("#eAbout").value = data.profile.about; $("#profilePreview").src = data.profile.profileImage;
 $("#eSkills").value = data.skills.join("\n");
 $("#eEmail").value = data.contact.email \vert{}\vert{} ""; $("#ePhone").value = data.contact.phone || "";
 $("#eFacebook").value = data.contact.facebook \vert{}\vert{} ""; $("#eLinkedin").value = data.contact.linkedin || "";
 $("#eGithub").value = data.contact.github \vert{}\vert{} ""; $("#eContactText").value = data.contact.text || "";
 ...
 $("#certEdit").innerHTML = data.certificates.length ? data.certificates.map((x, i) => certForm(x, i)).join("") : "";
}

function projectForm(x, i) {
 return `<div class="repeat"><div class="repeat-head"><b>Project ${i+1}</b><button class="small-btn danger" data-remove-project="${i}">Delete</button></div>
 <div class="row"><input data-p="${i}" data-k="title" value="${esc(x.title)}" placeholder="Title"><input data-p="${i}" data-k="category" value="${esc(x.category)}" placeholder="Category"></div>
 <textarea data-p="${i}" data-k="description" placeholder="Description">${esc(x.description)}</textarea>
 <input data-p="${i}" data-k="url" value="${esc(x.url||"")}" placeholder="Project URL">
 <input type="file" accept="image/*" data-project-file="${i}">
 <img class="upload-preview" src="${safeUrl(x.image)}"></div>`;
}

function timelineForm(x, i) {
 return `<div class="repeat"><div class="repeat-head"><b>Item ${i+1}</b><button class="small-btn danger" data-remove-time="${i}">Delete</button></div>
 <div class="row"><input data-t="${i}" data-k="type" value="${esc(x.type)}" placeholder="Experience/Education"><input data-t="${i}" data-k="title" value="${esc(x.title)}" placeholder="Title"><input data-t="${i}" data-k="period" value="${esc(x.period)}" placeholder="Period"></div>
 <textarea data-t="${i}" data-k="description" placeholder="Description">${esc(x.description)}</textarea></div>`;
}

function certForm(x, i) {
 return `<div class="repeat"><div class="repeat-head"><b>Certificate ${i+1}</b><button class="small-btn danger" data-remove-cert="${i}">Delete</button></div>
 <input data-c="${i}" data-k="title" value="${esc(x.title)}" placeholder="Certificate title"><input data-c="${i}" data-k="issuer" value="${esc(x.issuer||"")}" placeholder="Issuer">
 <textarea data-c="${i}" data-k="description" placeholder="Description">${esc(x.description||"")}</textarea>
 <input type="file" accept="image/*" data-cert-file="${i}">
 <img class="upload-preview" src="${safeUrl(x.image||"assets/certificate-placeholder.svg")}"></div>`;
}
...
function bindEditor() {
 ...
 $("#editor").onclick = e => {
  let b = e.target;
  if(b.dataset.removeProject) { data.projects.splice(+b.dataset.removeProject, 1); buildEditor() }
  if(b.dataset.removeTime) { data.timeline.splice(+b.dataset.removeTime, 1); buildEditor() }
  if(b.dataset.removeCert) { data.certificates.splice(+b.dataset.removeCert, 1); buildEditor() }
 };
...
async function saveAll() {
 ...
 try {
  ...
  console.error(e);
  msg.textContent = "Save failed. Check rules and setup.";
 }
}
