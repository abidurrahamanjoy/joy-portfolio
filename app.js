...
function renderProjects(cat) {
 ...
 $("#projectsGrid").innerHTML = arr.map(x => `<article class="card"><img class="card-img" src="${safeUrl(x.image)}" alt=""><div class="card-body"><p class="eyebrow">${esc(x.category || "Project")}</p><h3>${esc(x.title)}</h3><p>${esc(x.description)}</p>${x.url && x.url !== "#" ? `<a class="btn" href="${safeUrl(x.url)}" target="_blank">Open project</a>` : ""}</div></article>`).join("");
}

function esc(v = "") { return String(v).replace(/[&<>"']/g, c => ({"&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#039;"}[c])) }
function safeUrl(v = "") { return String(v).replace(/["'<>]/g, "") }

$("#projectFilters").addEventListener("click", e => { if(e.target.matches(".filter")) { $$(".filter").forEach(x => x.classList.remove("active")); e.target.classList.add("active"); renderProjects(e.target.dataset.cat); } });
...
onAuthStateChanged(auth, user => {
 if(user && user.uid === ADMIN_UID) { $("#loginView").classList.add("hidden"); $("#dashboardView").classList.remove("hidden"); buildEditor(); } 
 else { if (user) signOut(auth); $("#loginView").classList.remove("hidden"); $("#dashboardView").classList.add("hidden"); }
});

$("#loginBtn").onclick = async () => {
 try { const cred = await signInWithEmailAndPassword(auth, $("#email").value, $("#password").value); if(cred.user.uid !== ADMIN_UID) { await signOut(auth); throw new Error("UNAUTHORIZED_ADMIN"); } $("#loginMsg").textContent = ""; } 
 catch (e) { $("#loginMsg").textContent = e.message === "UNAUTHORIZED_ADMIN" ? "This account is not authorized." : "Login failed."; }
};
$("#logoutBtn").onclick = () => signOut(auth);

function buildEditor() {
 $("#editor").innerHTML = `<div class="editor-section"><h3>Profile</h3><label>Name<input id="eName"></label><label>Tagline<input id="eTagline"></label><label>About<textarea id="eAbout" rows="4"></textarea></label><label>Profile photo<input id="profileFile" type="file" accept="image/*"></label><img id="profilePreview" class="upload-preview"></div><div class="editor-section"><h3>Projects</h3><div id="projectEdit"></div><button class="small-btn" id="addProject">+ Add project</button></div><div class="editor-section"><h3>Skills</h3><textarea id="eSkills" rows="4"></textarea></div><div class="editor-section"><h3>Experience & Education</h3><div id="timelineEdit"></div><button class="small-btn" id="addTimeline">+ Add item</button></div><div class="editor-section"><h3>Certificates</h3><div id="certEdit"></div><button class="small-btn" id="addCert">+ Add certificate</button></div><div class="editor-section"><h3>Contact</h3><div class="row"><input id="eEmail" placeholder="Email"><input id="ePhone" placeholder="Phone"><input id="eFacebook" placeholder="Facebook URL"><input id="eLinkedin" placeholder="LinkedIn URL"><input id="eGithub" placeholder="GitHub URL"></div><textarea id="eContactText"></textarea></div><div class="save"><button class="btn primary full" id="saveAll">💾 Save all changes</button><p id="saveMsg" class="msg"></p></div>`;
 fillEditor(); bindEditor();
}

function fillEditor() {
 $("#eName").value = data.profile.name; $("#eTagline").value = data.profile.tagline; $("#eAbout").value = data.profile.about; $("#profilePreview").src = data.profile.profileImage; $("#eSkills").value = data.skills.join("\n"); $("#eEmail").value = data.contact.email \vert{}\vert{} ""; $("#ePhone").value = data.contact.phone || ""; $("#eFacebook").value = data.contact.facebook \vert{}\vert{} ""; $("#eLinkedin").value = data.contact.linkedin || ""; $("#eGithub").value = data.contact.github \vert{}\vert{} ""; $("#eContactText").value = data.contact.text || "";
...
async function saveAll() {
 ...
 data.profile.name = $("#eName").value; data.profile.tagline = $("#eTagline").value; data.profile.about = $("#eAbout").value; data.skills = $("#eSkills").value.split("\n").map(x => x.trim()).filter(Boolean); data.contact = { text: $("#eContactText").value, email: $("#eEmail").value, phone: $("#ePhone").value, facebook: $("#eFacebook").value, linkedin: $("#eLinkedin").value, github: $("#eGithub").value };
 document.querySelectorAll("[data-p]").forEach(el => { const i = +el.dataset.p; data.projects[i][el.dataset.k] = el.value }); document.querySelectorAll("[data-t]").forEach(el => { const i = +el.dataset.t; data.timeline[i][el.dataset.k] = el.value }); document.querySelectorAll("[data-c]").forEach(el => { const i = +el.dataset.c; data.certificates[i][el.dataset.k] = el.value });
 try {
...
async function uploadFile(file, prefix) {
 ...
 const form = new FormData(); form.append("file", file); form.append("upload_preset", cloudinaryConfig.uploadPreset); form.append("tags", `portfolio,${prefix}`);
 ...
 const out = await res.json(); return out.secure_url;
}
